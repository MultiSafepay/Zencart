...
class multisafepay_beautyandwellness extends multisafepay
{
    ...
    function __construct()
    {
        ...
        $this->public_title = $this->getTitle(MODULE_PAYMENT_MSP_BEAUTYANDWELLNESS_TEXT_TITLE);
        $this->description = $this->description = "<img src='images/icon_info.gif' border='0'>&nbsp;<b>MultiSafepay Beauty & Wellness Cadeau</b><BR>The main MultiSafepay module must be installed (does not have to be active) to use this payment method.<BR>";
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_MSP_BEAUTYANDWELLNESS_ZONE > 0)) {
            $check_flag = false;
            $check_query = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MSP_BEAUTYANDWELLNESS_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function process_button()
    {
        ...
        return zen_draw_hidden_field('msp_paymentmethod', 'BEAUTYANDWELLNESS');
    }
    ...
    function check()
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_MSP_BEAUTYANDWELLNESS_STATUS'");
