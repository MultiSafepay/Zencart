...
class multisafepay_einvoice extends MultiSafepay
{
    ...
    public function update_status()
    {
        ...
        if (($this->enabled == true) && ((int)MODULE_PAYMENT_MSP_EINVOICE_ZONE > 0)) {
            $check_flag = false;
            $check_query = $db->Execute("SELECT zone_id FROM " . TABLE_ZONES_TO_GEO_ZONES . " WHERE geo_zone_id = '" . MODULE_PAYMENT_MSP_EINVOICE_ZONE . "' AND zone_country_id = '" . $order->billing['country']['id'] . "' ORDER BY zone_id");
        ...
        if (isset($order->info['total'])) {
            ...
            $min_amount = (float)MODULE_PAYMENT_MSP_EINVOICE_MIN_AMOUNT;
            $max_amount = (float)MODULE_PAYMENT_MSP_EINVOICE_MAX_AMOUNT;
    ...
    {
        return (
            zen_draw_hidden_field('msp_paymentmethod', 'EINVOICE') .
    ...
    public function check()
    {
        ...
        if (!isset($this->_check)) {
            ...
            $checkQuery = $db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_MSP_EINVOICE_STATUS'");
