...
class multisafepay_ing extends multisafepay
{
    ...
    function check()
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_MSP_ING_STATUS'");
    ...
    function keys()
    {
        return array
            (
            'MODULE_PAYMENT_MSP_ING_STATUS',
            'MODULE_PAYMENT_MSP_ING_SORT_ORDER',
            'MODULE_PAYMENT_MSP_ING_ZONE'
        );
    }
