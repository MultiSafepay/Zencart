...
class multisafepay_paysafecard extends multisafepay {
    ...
    function __construct()
    {
        ...
        $this->title = $this->getTitle(MODULE_PAYMENT_MSP_PAYSAFECARD_TEXT_TITLE);
        $this->public_title = $this->getTitle(MODULE_PAYMENT_MSP_PAYSAFECARD_TEXT_TITLE);
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_MSP_PAYSAFECARD_ZONE > 0)) {
            $check_flag = false;
            $check_query = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MSP_PAYSAFECARD_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function check()
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_MSP_PAYSAFECARD_STATUS'");
    ...
    function install()
    {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable MultiSafepay PaySafeCard Module', 'MODULE_PAYMENT_MSP_PAYSAFECARD_STATUS', 'True', 'Do you want to accept PaySafeCard payments?', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_MSP_PAYSAFECARD_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
