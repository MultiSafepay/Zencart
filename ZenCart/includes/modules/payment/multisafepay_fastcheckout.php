...
class multisafepay_fastcheckout {
    ...
    function checkout_notify()
    {
        ...
        try {
            ...
            $response_obj = $msp->orders->get('orders', $this->order_id);
        ...
        $customer_country = $this->get_country_from_code($response_obj->customer->country);
        $delivery_country = $this->get_country_from_code($response_obj->delivery->country);

        $shippers = $this->get_shipping_methods(0);
        $shipper = explode('-', $response_obj->order_adjustment->shipping->flat_rate_shipping->name);
        ...
        $sql_data_array = array
            (
            'customers_name' => $response_obj->customer->first_name . ' ' . $response_obj->customer->last_name,
            'customers_company' => '',
            'customers_street_address' => $response_obj->customer->address1 . ' ' . $response_obj->customer->house_number,
            'customers_suburb' => '',
            'customers_city' => $response_obj->customer->city,
            'customers_postcode' => $response_obj->customer->zip_code,
            'customers_state' => $response_obj->customer->state,
            'customers_country' => $customer_country->fields['countries_name'],
            'customers_telephone' => $response_obj->customer->phone1,
            'customers_email_address' => $response_obj->customer->email,
            'customers_address_format_id' => '1',
            'delivery_name' => $response_obj->delivery->first_name . ' ' . $response_obj->delivery->last_name,
            'delivery_company' => '',
            'delivery_street_address' => $response_obj->delivery->address1 . ' ' . $response_obj->delivery->house_number,
            'delivery_suburb' => '',
            'delivery_city' => $response_obj->delivery->city,
            'delivery_postcode' => $response_obj->delivery->zip_code,
            'delivery_state' => $response_obj->delivery->state,
            'delivery_country' => $delivery_country->fields['countries_name'],
            'delivery_address_format_id' => '1',
            'billing_name' => $response_obj->customer->first_name . ' ' . $response_obj->customer->last_name,
            'billing_company' => '',
            'billing_street_address' => $response_obj->customer->address1 . ' ' . $response_obj->customer->house_number,
            'billing_suburb' => '',
            'billing_city' => $response_obj->customer->city,
            'billing_postcode' => $response_obj->customer->zip_code,
            'billing_state' => $response_obj->customer->state,
            'billing_country' => $customer_country->fields['countries_name'],
            'billing_address_format_id' => '1',
            'payment_method' => 'MultiSafepay FastCheckout (' . $response_obj->payment_details->type . ')',
            'payment_module_code' => 'multisafepay_fastcheckout',
            'order_tax' => $response_obj->order_adjustment->total_tax,
            'order_total' => $response_obj->order_total,
            'shipping_method' => $response_obj->order_adjustment->shipping->flat_rate_shipping->name,
            'shipping_module_code' => $shipping_module_code
        );

        $order->customer['firstname'] = $response_obj->customer->first_name;
        $order->customer['lastname'] = $response_obj->customer->last_name;
        ...
        $value = $response_obj->order_total;
        ...
        //Update or add tax
        $value = $response_obj->order_adjustment->total_tax;
        ...
        $value = $response_obj->order_adjustment->shipping->flat_rate_shipping->cost;
        $title = $response_obj->order_adjustment->shipping->flat_rate_shipping->name;
        $text = $currencies->format($value, false, $GLOBALS['order']->info['currency'], $currencies->currencies[$currency]['value']);
