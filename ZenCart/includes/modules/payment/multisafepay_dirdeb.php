...
class multisafepay_dirdeb extends multisafepay
{
    ...
    if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_MSP_DIRDEB_ZONE > 0) ) {
      $check_flag = false;
      $check_query = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MSP_DIRDEB_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
	...
	function process_button()
	{
		...
		return zen_draw_hidden_field('msp_paymentmethod', 'DIRDEB');
	...
	function check()
	{
		...
		{
			$check_query 	= 	$db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_MSP_DIRDEB_STATUS'");
