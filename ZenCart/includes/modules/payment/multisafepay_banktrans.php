...
class multisafepay_banktrans extends multisafepay
{
    ...
    if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_MSP_BANKTRANS_ZONE > 0) ) {
      $check_flag = false;
      $check_query = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MSP_BANKTRANS_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
	...
	function install()
	{
		...
		//$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Direct iDeal', 'MODULE_PAYMENT_MSP_BANKTRANS_DIRECT', 'True', 'Select the bank within the website?', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
	}
