...
class multisafepay_paypal extends multisafepay
{
	...
	function process_button()
	{
		...
		return zen_draw_hidden_field('msp_paymentmethod', 'PAYPAL');
	...
	function check()
	{
		...
		{
			$check_query 	= 	$db->Execute("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_MSP_PAYPAL_STATUS'");
	...
	{
		global $db;
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable MultiSafepay PAYPAL Module', 'MODULE_PAYMENT_MSP_PAYPAL_STATUS', 'True', 'Do you want to accept PAYPAL payments?', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_MSP_PAYPAL_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
		...
		//$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Direct iDeal', 'MODULE_PAYMENT_MSP_PAYPAL_DIRECT', 'True', 'Select the bank within the website?', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
	}
